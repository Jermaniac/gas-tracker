export const TOTAL_PROVINCES_PAGES : number = 7

export const ITEMS_PROVINCES_PER_PAGE : number = 8

export const FUEL_TYPES : Record<string, string> = {
    gas95E5: "Gasoline 95 E5",
    gas98E5: "Gasoline 98 E5",
    gas95E10: "Gasoline 95 E10",
    gas98E10: "Gasoline 98 E10",
    biodiesel: "Biodiesel",
    bioetanol: "Bioethanol",
    gasNaturalComprimido: "Compressed Natural Gas",
    gasNaturalLicuado: "Liquefied Natural Gas",
    gasesLicuadosPetroleo: "Liquefied Petroleum Gas",
    gasoleoA: "Diesel A",
    gasoleoB: "Diesel B",
    gasoleoPremium: "Premium Diesel",
    hydrogen: "Hydrogen",
    gas95E5Premium: "Gasoline 95 E5 Premium"
};
